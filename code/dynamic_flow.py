...
class DynamicFlow:
    ...
    #puts out some information about the dynamic flow like inflows and outflow and queues
    def info(self):
        print("Information about the flow")
        for i in range(len(self._network.graph.edges)):
            for j in range(len(self._network.paths)):
                if self._network.graph.edges[i] in self._network.paths[j] and j in self.inflow[i]._functions_dict:
        ...
        for i in range(len(self._network.graph.edges)):
            ...
            domain = self.queues[i].domain
            print("The queue length function on edge " + str(i) + " has the times " + str(times) + " and the values " + str(values) + " and the domain " + str(domain))